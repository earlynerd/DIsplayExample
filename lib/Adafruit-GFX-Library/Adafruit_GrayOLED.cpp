...
/*!
    @brief  Adjust the display current 
    @param  level The contrast level from 0 to 0xFF
    @note   This has an immediate effect on the display, no need to call the
            display() function -- buffer contents are not changed.
*/
void Adafruit_GrayOLED::setMasterCurrent(uint8_t level) {
  uint8_t cmd[] = {GRAYOLED_SETMASTERCURRENT, level};
  oled_commandList(cmd, 2);
}

/*!
    @brief  Adjust the precharge voltage
    @param  level The voltage level from 0 to 0x1F
    @note   This has an immediate effect on the display, no need to call the
            display() function -- buffer contents are not changed.
*/
void Adafruit_GrayOLED::setPreChargeVoltage(uint8_t level) {
  uint8_t cmd[] = {GRAYOLED_SETPRECHARGEVOLTAGE, level};
  oled_commandList(cmd, 2);
}
